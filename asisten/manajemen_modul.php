...
$activePage = 'modul';
require_once 'templates/header.php'; // ✅ PAKAI NAMA YANG ADA
include '../config.php';
    ...
    <?php if ($editMode): ?>
      ...
      <form method="POST" enctype="multipart/form-data">
        ...
        <?php if ($editData['file_materi']): ?>
          <p class="text-sm">File: <a href="../uploads/<?= $editData['file_materi'] ?>" target="_blank" class="text-blue-600 underline"><?= htmlspecialchars($editData['file_materi']) ?></a></p>
        <?php endif; ?>

        <button type="submit" name="update" class="bg-blue-500 text-white px-4 py-2 rounded">Simpan Perubahan</button>
        <a href="manajemen_modul.php" class="ml-2 text-red-500">Batal</a>
      </form>
    <?php else: ?>
    ...
    <tbody>
      <?php foreach ($modul as $row): ?>
        <tr>
          ...
          <td class="border p-2">
            <a href="manajemen_modul.php?edit=<?= $row['id'] ?>" class="bg-yellow-500 text-white px-2 py-1 rounded">Edit</a>
            <a href="manajemen_modul.php?delete=<?= $row['id'] ?>" onclick="return confirm('Yakin hapus modul?')" class="bg-red-500 text-white px-2 py-1 rounded">Hapus</a>
          </td>
        </tr>
      <?php endforeach; ?>
    </tbody>
